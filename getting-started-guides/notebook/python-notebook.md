...
2. Make sure you have `SPARK_JARS` and `SPARK_PY_FILES` set properly. Please note, here *cudf-0.14-cuda10-1.jar* is used as an example. Please choose other *cudf-0.14* jars based on your environment. You may need to update these env variables because the working directory will be changed:
  ```
  export LIBS_PATH=[full path to xgboost4j_spark/libs]
  export SPARK_JARS=${LIBS_PATH}/cudf-0.14-cuda10-1.jar,${LIBS_PATH}/xgboost4j_3.0-1.0.0-0.1.0.jar,${LIBS_PATH}/xgboost4j-spark_3.0-1.0.0-0.1.0.jar,${LIBS_PATH}/rapids-4-spark_2.12-0.1.0.jar
